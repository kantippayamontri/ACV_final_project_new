...
def read_features(lmdb_path: str, sentence_name: str):
    env = lmdb.open(lmdb_path, readonly=True, subdir=False, lock=False, max_readers=126, meminit=False)
    try:
...
def run(args):
    ...
    for row in rows:
        ...
        features = read_features(args.lmdb, row["SENTENCE_NAME"])
        if features.numel() == 0:
            previous_prediction = ""
            continue
